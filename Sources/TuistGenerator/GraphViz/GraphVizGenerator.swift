...
public protocol GraphVizGenerating {
    ...
    func generateProject(at path: AbsolutePath, skipTestTargets: Bool, skipExternalDependencies: Bool, targetsToFilter: [String]) throws -> GraphViz.Graph
    ...
    func generateWorkspace(at path: AbsolutePath, skipTestTargets: Bool, skipExternalDependencies: Bool, targetsToFilter: [String]) throws -> GraphViz.Graph
}
...
public final class GraphVizGenerator: GraphVizGenerating {
    ...
    /// - Throws: An error if the project can't be loaded.
    public func generateProject(at path: AbsolutePath, skipTestTargets: Bool, skipExternalDependencies: Bool, targetsToFilter: [String]) throws -> GraphViz.Graph {
        let (graph, _) = try graphLoader.loadProject(path: path)
        return graphToGraphVizMapper.map(graph: graph, skipTestTargets: skipTestTargets, skipExternalDependencies: skipExternalDependencies, targetsToFilter: targetsToFilter)
    }
    ...
    /// - Returns: Dot graph representation.
    /// - Throws: An error if the workspace can't be loaded.
    public func generateWorkspace(at path: AbsolutePath, skipTestTargets: Bool, skipExternalDependencies: Bool, targetsToFilter: [String]) throws -> GraphViz.Graph {
